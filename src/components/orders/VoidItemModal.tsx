...
const VoidItemModal = ({ item, onConfirm, onClose }: VoidItemModalProps) => {
  ...
  const handleConfirm = () => {
    const finalReason = reason === 'Other' ? `Other: ${otherText.trim() || 'No details'}` : reason;
    if (!reason) return;
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div>
            <p className="text-[11px] font-black uppercase tracking-widest mb-1" style={{ color: 'rgba(239,68,68,0.8)' }}>
              🚫 Void / Cancel Item
            </p>
        ...
        <div className="px-5 py-4 space-y-5">
          {/* Quantity selector */}
          <div>
            <p className="text-xs font-bold uppercase tracking-wide mb-2.5" style={{ color: 'rgba(148,163,184,0.7)' }}>
              How many to cancel?
            </p>
        ...
        >
          ...
          <button
            ...
            style={{ background: canConfirm ? 'rgba(239,68,68,0.85)' : 'rgba(239,68,68,0.3)', color: '#fff', boxShadow: canConfirm ? '0 4px 16px -4px rgba(239,68,68,0.55)' : 'none' }}
          >
            Void {qty} × {item.name.split(' ')[0]}
          </button>
        </div>
      </div>
    </div>
