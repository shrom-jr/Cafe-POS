...
const ProductForm = ({ editProduct, onClose }: ProductFormProps) => {
  ...
  const [form, setForm] = useState(() =>
    editProduct ? {
      ...
      currentStockBottles: String(editProduct.currentStockMl / editProduct.bottleSizeMl),
  ...
  const handleSave = () => {
    ...
    const data: Omit<AlcoholProduct, 'id'> = {
      ...
      currentStockMl: Math.round(csb * bsz),
  ...
  return (
    <div className={CARD}>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        {/* Product Name — full width on mobile, half on sm */}
        <div className="col-span-2 sm:col-span-1">
          <label className={LABEL}>Product Name *</label>
          <input className={INPUT} placeholder="e.g. Ruslan Vodka" value={form.name} onChange={f('name')} autoFocus />
        </div>

        {/* Bottle Size */}
        <div>
          <label className={LABEL}>Bottle Size (ml) *</label>
          <input className={INPUT} type="number" min="1" step="1" placeholder="750"
            value={form.bottleSizeMl} onChange={f('bottleSizeMl')} />
        </div>

        {/* Current Stock */}
        <div>
          <label className={LABEL}>Current Stock (bottles) *</label>
          <input className={INPUT} type="number" min="0" step="0.5" placeholder="10"
            value={form.currentStockBottles} onChange={f('currentStockBottles')} />
          {currentMlPreview !== null && (
            <p className="text-xs text-blue-400/80 mt-1">
              = {currentMlPreview.toLocaleString()} ml total stock
            </p>
          )}
        </div>
        ...
        <div>
          ...
          {minMlPreview !== null && (
            <p className="text-xs text-muted-foreground/60 mt-1">
              = {minMlPreview.toLocaleString()} ml alert threshold
            </p>
          )}
        </div>
        ...
        <div>
          ...
          <input className={INPUT} type="number" min="0" step="any" placeholder="e.g. 2000"
            value={form.costPerBottle} onChange={f('costPerBottle')} />
        </div>
        ...
        <div>
          ...
          <select className={SELECT} value={form.status}
            onChange={f('status') as React.ChangeEventHandler<HTMLSelectElement>}>
      ...
      <div className="flex gap-2 mt-4">
        <button className={BTN_PRIMARY} onClick={handleSave}>
          <Save size={14} /> {editProduct ? 'Save Changes' : 'Add Product'}
        </button>
...
const PurchaseForm = ({ product, onClose }: PurchaseFormProps) => {
  ...
  const handleSave = () => {
    ...
    toast.success(`+${addMl?.toLocaleString()} ml added to ${product.name}`);
    onClose();
  };
    ...
    <div className={`${CARD} border-blue-500/20 bg-blue-500/[0.03]`}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div>
          ...
          <input className={INPUT} type="number" min="0.5" step="0.5" placeholder="12"
            value={form.bottles} onChange={f('bottles')} autoFocus />
          {addMl !== null && (
        ...
        <div>
          ...
          <input className={INPUT} type="number" min="0" step="any" placeholder="2000"
            value={form.costPerBottle} onChange={f('costPerBottle')} />
        </div>
      </div>
      <div className="flex gap-2 mt-4">
...
const EMPTY_ADJ = {
  ...
  amountMl: '',
  type: 'Adjustment' as InvMovementType,
  reason: '',
};
...
const AdjustForm = ({ product, onClose }: AdjustFormProps) => {
  ...
  const handleSave = () => {
    ...
    toast.success(`Stock adjusted: ${form.direction === 'add' ? '+' : '-'}${ml.toLocaleString()} ml`);
    onClose();
  };
    ...
    <div className={`${CARD} border-yellow-500/20 bg-yellow-500/[0.03]`}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div>
          ...
          <input className={INPUT} type="number" min="1" step="any" placeholder="750"
            value={form.amountMl}
...
export const AlcoholSection = () => {
  ...
  const handleDelete = (p: AlcoholProduct) => {
    if (!confirm(`Delete "${p.name}"? This also removes all its POS mappings.`)) return;
    deleteAlcohol(p.id);
